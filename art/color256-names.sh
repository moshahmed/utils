# What: Print and show 256 colors and names on terminal
...
cat $0 |
grep -Pis '#(\S+)\s+(rgb)' |
while read -r hash ci cn cx crgb chsl ; do
    printf "|\x1b[48;5;%sm%3d\e[0m"  $ci $ci
    printf  "\x1b[38;5;%sm%3d\e[0m"  $ci $ci
    printf  "\x1b[48;5;%sm%18s\e[0m|" $ci $cn
    nl=$(($ci%4))
    # linelen=$(($linelen + ${#ci}+${#ci} + ${#cn} ))
    if [[ $nl == 3 ]] ;then
      # linelen=0
      printf "\n"
    fi
